...
function toLocalRelative(
  ...
  if (pathMappings && pathMappings.length > 0) {
    return reversePathMappings(devicePath, rootPath, pathMappings);
  }
...
export class BoardOperations {
  ...
  async syncBaselineFromBoard(): Promise<void> {
    ...
    const rootPath = vscode.workspace
      .getConfiguration()
      .get<string>("mpyWorkbench.rootPath", "/");
    const deviceStats = await this.withAutoSuspend(() =>
  ...
  private async generateComparisonPlan(rootPath: string): Promise<void> {
    ...
    try {
      ...
      await fs.writeFile(
        planFilePath,
        JSON.stringify(comparisonPlan, null, 2),
        "utf8",
      );

      console.log(
        `[DEBUG] checkDiffs: Comparison plan saved to: ${planFilePath}`,
      );
      console.log(
        `[DEBUG] checkDiffs: Plan includes ${comparisonPlan.comparisons.length} file operations`,
      );

      // Show summary in console
      const compareCount = comparisonPlan.comparisons.filter(
        (c) => c.type === "comparison",
      ).length;
      const localOnlyCount = comparisonPlan.comparisons.filter(
        (c) => c.type === "local_only",
      ).length;
      const boardOnlyCount = comparisonPlan.comparisons.filter(
        (c) => c.type === "board_only",
      ).length;
      ...
      console.error(
        `[DEBUG] checkDiffs: Failed to generate comparison plan: ${error.message}`,
      );
    }
  }
  ...
  private toDevicePath(
    ...
    if (pathMappings && pathMappings.length > 0) {
      return applyPathMappings(localRel, rootPath, pathMappings);
    }
  ...
  async checkDiffs(): Promise<void> {
    await vscode.window.withProgress(
      ...
      async (progress) => {
        ...
        if (pathMappings) {
          console.log(
            `[DEBUG] checkDiffs: pathMappings: ${JSON.stringify(pathMappings)}`,
          );
        }
        ...
        async function collectLocalDirs(
          dir: string,
          relBase = "",
        ): Promise<void> {
        ...
        const filteredBoardFiles = new Map<
          string,
          { size: number; isDir: boolean }
        >();
        ...
        for (const localRel of localFiles) {
          ...
          if (boardFile) {
            ...
            // File exists locally but not on board
            const devicePath = this.toDevicePath(
              localRel,
              rootPath,
              pathMappings,
            );
            console.log(
              `[DEBUG] checkDiffs: Adding local-only: localRel=${localRel}, devicePath=${devicePath}`,
            );
        ...
        for (const localDir of localDirectories) {
          ...
          if (!existsOnBoard) {
            console.log(
              `[DEBUG] checkDiffs: Adding local-only directory: localDir=${localDir}, devicePath=${devicePath}`,
            );
        ...
        (this.decorations as any)._originalLocalOnlyDirectories =
  ...
  async syncDiffsLocalToBoard(noClear: boolean = false): Promise<void> {
    ...
    console.log(
      ...
      this.decorations.getDiffsFilesOnly(),
    );
    console.log(
      "- decorations.getLocalOnlyFilesOnly():",
      this.decorations.getLocalOnlyFilesOnly(),
    );
      ...
      async (progress) => {
        ...
        await this.withAutoSuspend(async () => {
          for (const devicePath of allFilesToSync) {
            console.log(
              `[DEBUG] syncDiffsLocalToBoard: Processing devicePath: ${devicePath}, rootPath: ${rootPath}`,
            );
            ...
            try {
              ...
              if (stat.isDirectory()) {
                console.log(
                  `[DEBUG] syncDiffsLocalToBoard: Skipping directory: ${abs}`,
                );
                continue;
              }
            } catch (accessError: any) {
              console.error(
                `[DEBUG] syncDiffsLocalToBoard: File not accessible: ${abs}, error: ${accessError.message}`,
              );
              continue;
            }
            ...
            try {
              ...
              console.log(
                `[DEBUG] syncDiffsLocalToBoard: Successfully uploaded: ${abs} -> ${devicePath}`,
              );
            } catch (uploadError: any) {
              console.error(
                `[DEBUG] syncDiffsLocalToBoard: Failed to upload ${abs} -> ${devicePath}, error: ${uploadError.message}`,
              );
              // Continue with next file instead of failing completely
            }
          }
    ...
    if (ws) {
      ...
      if (isLocalOnly) {
        ...
        if (!fileExistsLocally) {
          console.log(
            `[DEBUG] openFile: File not found locally, copying from board: ${node.path} -> ${abs}`,
          );
          try {
            await this.withAutoSuspend(() => mp.cpFromDevice(node.path, abs));
            console.log(
              `[DEBUG] openFile: Successfully copied file from board`,
            );
          } catch (copyError: any) {
            console.error(
              `[DEBUG] openFile: Failed to copy file from board:`,
              copyError,
            );
            vscode.window.showErrorMessage(
      ...
      try {
        ...
        if (stats.size === 0) {
          console.warn(
            `[DEBUG] openFile: Local file is empty, this might indicate a copy failure`,
          );
          vscode.window.showWarningMessage(
            `File appears to be empty. The copy from board may have failed.`,
          );
        }
        ...
        console.error(
          `[DEBUG] openFile: Failed to open local file:`,
          openError,
        );
        vscode.window.showErrorMessage(
          `Failed to open file: ${openError?.message || openError}`,
        );
      }
    } else {
      ...
      try {
        ...
        console.error(
          `[DEBUG] openFile: Failed to copy file to temp location:`,
          copyError,
        );
        vscode.window.showErrorMessage(
