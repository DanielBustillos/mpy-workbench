...
export function activate(context: vscode.ExtensionContext) {
  ...
  async function ensureWorkbenchIgnoreFile(wsPath: string) {
    try {
      ...
      try {
        await fs.writeFile(
          path.join(wsPath, MPY_WORKBENCH_DIR, ".mpyignore"),
          content,
          "utf8",
        );
      } catch {}
  ...
  async function workspaceAutoSyncEnabled(wsPath: string): Promise<boolean> {
    ...
    return vscode.workspace
      .getConfiguration()
      .get<boolean>("mpyWorkbench.autoSyncOnSave", false);
  }
  ...
  vscode.workspace
    .getConfiguration()
    .update("mpyWorkbench.connect", "auto", vscode.ConfigurationTarget.Global);
  updatePortContext();

  const tree = new Esp32Tree();
  const view = vscode.window.createTreeView("mpyWorkbenchFsView", {
    treeDataProvider: tree,
  });
  const actionsTree = new ActionsTree();
  const actionsView = vscode.window.createTreeView("mpyWorkbenchActionsView", {
    treeDataProvider: actionsTree,
  });
  const syncTree = new SyncTree();
  const syncView = vscode.window.createTreeView("mpyWorkbenchSyncView", {
    treeDataProvider: syncTree,
  });
  ...
  const autoSyncStatus = vscode.window.createStatusBarItem(
  ...
  const cancelTasksStatus = vscode.window.createStatusBarItem(
    vscode.StatusBarAlignment.Right,
    90,
  );
  ...
  cancelTasksStatus.color = new vscode.ThemeColor(
  ...
  if (
    ...
    const cfgGlob = new vscode.RelativePattern(
      wsPath,
      ".mpystudio/config.json",
    );
  ...
  context.subscriptions.push(
    ...
    vscode.commands.registerCommand(
      "mpyWorkbench.refreshFileTreeCache",
      async () => {
        try {
          ...
          vscode.window.showInformationMessage(
            "File tree cache refreshed successfully",
          );
        } catch (error: any) {
          console.error("[DEBUG] File tree cache refresh failed:", error);
          vscode.window.showErrorMessage(
            `File tree cache refresh failed: ${error?.message || error}`,
          );
        }
      },
    ),
    vscode.commands.registerCommand(
      "mpyWorkbench.rebuildManifest",
      async () => {
        try {
          ...
          vscode.window.showInformationMessage(
            `Manifest rebuilt successfully (${Object.keys(newManifest.files).length} files)`,
          );
        } catch (error: any) {
          console.error("[DEBUG] Manifest rebuild failed:", error);
          vscode.window.showErrorMessage(
            `Manifest rebuild failed: ${error?.message || error}`,
          );
        }
      },
    ),
    vscode.commands.registerCommand(
      "mpyWorkbench.debugTreeParsing",
      async () => {
        try {
          ...
          vscode.window.showErrorMessage(
            `Tree parsing debug failed: ${error?.message || error}`,
          );
        }
      },
    ),
    vscode.commands.registerCommand(
      "mpyWorkbench.debugFilesystemStatus",
      async () => {
        try {
          ...
          vscode.window.showErrorMessage(
            `Filesystem status debug failed: ${error?.message || error}`,
          );
        }
      },
    ),
    vscode.commands.registerCommand("mpyWorkbench.cancelAllTasks", async () => {
      try {
        ...
        vscode.window.showErrorMessage(
          `Failed to cancel tasks: ${error?.message || error}`,
        );
      }
    }),
    ...
    vscode.commands.registerCommand(
      "mpyWorkbench.newFileBoardAndLocal",
      async () => {
        const ws = vscode.workspace.workspaceFolders?.[0];
        if (!ws) {
          vscode.window.showErrorMessage("No workspace folder open");
        ...
        const rootPath = vscode.workspace
          .getConfiguration()
          .get<string>("mpyWorkbench.rootPath", "/");
          ...
          async (savedDoc) => {
            ...
            try {
              ...
              if (matcher(rel.replace(/\\/g, "/"), false)) {
                vscode.window.showInformationMessage(
                  `File saved (ignored for upload): ${filename}`,
                );
              } else {
                try {
                  await withAutoSuspend(() => mp.cpToDevice(abs, devicePath));
                  vscode.window.showInformationMessage(
                    `File saved locally and uploaded to board: ${filename}`,
                  );
                  tree.addNode(devicePath, false);
                } catch (uploadError: any) {
                  console.error(
                    `[DEBUG] Failed to upload new file to board:`,
                    uploadError,
                  );
                  vscode.window.showWarningMessage(
                    `File saved locally but upload to board failed: ${uploadError?.message || uploadError}`,
                  );
                }
              }
            } catch (err: any) {
              vscode.window.showErrorMessage(
                `Error uploading file to board: ${err?.message ?? err}`,
              );
            }
    ...
    vscode.commands.registerCommand(
      "mpyWorkbench.openFileFromLocal",
      async (node: Esp32Node) => {
        ...
        try {
          ...
          vscode.window.showErrorMessage(
            `File not found in local workspace: ${toLocalRelative(node.path, vscode.workspace.getConfiguration().get<string>("mpyWorkbench.rootPath", "/"))}`,
          );
        }
      },
    ),
    vscode.commands.registerCommand(
      "mpyWorkbench.syncFileLocalToBoard",
      async (node: Esp32Node) => {
        if (node.kind !== "file") return;
        const ws = vscode.workspace.workspaceFolders?.[0];
        if (!ws) {
        ...
        try {
          ...
          const pick = await vscode.window.showWarningMessage(
            `Local file not found: ${rel}. Download from board first?`,
            { modal: true },
            "Download",
          );
    ...
    vscode.commands.registerCommand(
      "mpyWorkbench.syncFileBoardToLocal",
      async (node: Esp32Node) => {
        if (node.kind !== "file") return;
        const ws = vscode.workspace.workspaceFolders?.[0];
        if (!ws) {
    ...
    vscode.commands.registerCommand("mpyWorkbench.syncBaseline", async () => {
      try {
        ...
        if (!initialized) {
          ...
          vscode.window.showInformationMessage(
            "Local folder initialized for synchronization.",
          );
        }
        ...
        const wsConfig = await readWorkspaceConfig(ws.uri.fsPath);
        ...
        await vscode.window.withProgress(
          ...
          async (progress, token) => {
            ...
            await withAutoSuspend(async () => {
              ...
              const sortedDirectories = getAllMappedDirectories(
                files,
                rootPath,
                pathMappings,
              );
              ...
              console.log(
                `[DEBUG] syncBaseline: Need to create ${sortedDirectories.length} directories:`,
                sortedDirectories,
              );
              ...
              console.log(
                `[DEBUG] syncBaseline: Starting directory creation for ${sortedDirectories.length} directories...`,
              );
              ...
              for (const deviceDir of sortedDirectories) {
                ...
                while (!created && attempts < maxAttempts) {
                  ...
                  try {
                    console.log(
                      `[DEBUG] syncBaseline: Creating directory ${deviceDir} (attempt ${attempts}/${maxAttempts})`,
                    );
                    ...
                    createdCount++;
                    console.log(
                      `[DEBUG] syncBaseline: ✓ Created directory ${deviceDir} (${createdCount}/${sortedDirectories.length})`,
                    );
                  } catch (error: any) {
                    console.log(
                      `[DEBUG] syncBaseline: ✗ Directory ${deviceDir} creation failed (attempt ${attempts}):`,
                      error.message,
                    );
                    ...
                    if (attempts >= maxAttempts) {
                      ...
                      console.error(
                        `[DEBUG] syncBaseline: ✗✗ Giving up on directory ${deviceDir} after ${maxAttempts} attempts`,
                      );
                    } else {
              ...
              console.log(
                `[DEBUG] syncBaseline: Directory creation completed. Created ${createdCount} out of ${sortedDirectories.length} directories.`,
              );

              if (failedDirectories.length > 0) {
                console.error(
                  `[DEBUG] syncBaseline: Failed to create ${failedDirectories.length} directories:`,
                  failedDirectories,
                );
              }
              ...
              console.log(
                `[DEBUG] syncBaseline: Verifying ALL directories exist before bulk upload...`,
              );
              ...
              for (const deviceDir of sortedDirectories) {
                try {
                  ...
                  if (!exists) {
                    console.error(
                      `[DEBUG] syncBaseline: ✗ Directory ${deviceDir} does not exist!`,
                    );
                    verificationFailures.push(deviceDir);
                    allDirectoriesExist = false;
                  } else {
                    console.log(
                      `[DEBUG] syncBaseline: ✓ Directory ${deviceDir} verified`,
                    );
                  }
                } catch (error: any) {
                  console.error(
                    `[DEBUG] syncBaseline: ✗ Error checking directory ${deviceDir}:`,
                    error.message,
                  );
                  verificationFailures.push(deviceDir);
                  allDirectoriesExist = false;
              ...
              if (!allDirectoriesExist) {
                console.error(
                  `[DEBUG] syncBaseline: Cannot proceed with bulk upload - ${verificationFailures.length} directories missing:`,
                  verificationFailures,
                );

                // Try to create the missing directories one more time
                console.log(
                  `[DEBUG] syncBaseline: Attempting to create missing directories...`,
                );
                for (const missingDir of verificationFailures) {
                  try {
                    console.log(
                      `[DEBUG] syncBaseline: Creating missing directory: ${missingDir}`,
                    );
                    await mp.mkdir(missingDir);
                    tree.addNode(missingDir, true);
                    console.log(
                      `[DEBUG] syncBaseline: ✓ Successfully created missing directory: ${missingDir}`,
                    );
                  } catch (createError: any) {
                    console.error(
                      `[DEBUG] syncBaseline: ✗ Failed to create missing directory ${missingDir}:`,
                      createError.message,
                    );
                  }
                }
                ...
                console.log(
                  `[DEBUG] syncBaseline: Re-verifying directories after retry...`,
                );
                let stillMissing = [];
                for (const missingDir of verificationFailures) {
                  try {
                    ...
                    console.error(
                      `[DEBUG] syncBaseline: Error checking ${missingDir} after retry:`,
                      error.message,
                    );
                    stillMissing.push(missingDir);
                  }
                ...
                if (stillMissing.length > 0) {
                  console.error(
                    `[DEBUG] syncBaseline: Still missing ${stillMissing.length} directories after retry:`,
                    stillMissing,
                  );
                  throw new Error(
                    `Missing directories after retry: ${stillMissing.join(", ")}`,
                  );
                }

                console.log(
                  `[DEBUG] syncBaseline: ✓ All directories now exist after retry`,
                );
              }

              console.log(
                `[DEBUG] syncBaseline: ✓ All directories verified - proceeding with bulk upload`,
              );
              ...
              console.log(
                `[DEBUG] syncBaseline: Using individual cp commands for upload`,
              );
              ...
              for (const relativePath of files) {
                ...
                try {
                  ...
                  console.log(
                    `[DEBUG] syncBaseline: ✓ Local file exists: ${localPath}`,
                  );
                } catch (error) {
                  console.error(
                    `[DEBUG] syncBaseline: ✗ Local file missing: ${localPath}`,
                  );
              ...
              console.log(
                `[DEBUG] syncBaseline: ${validFiles.length}/${files.length} local files are accessible`,
              );

              // Warn user about missing files
              if (missingFiles.length > 0) {
                console.warn(
                  `[DEBUG] syncBaseline: Skipping ${missingFiles.length} missing files:`,
                  missingFiles.slice(0, 5),
                );
                if (missingFiles.length > 5) {
                  console.warn(
                    `[DEBUG] syncBaseline: ... and ${missingFiles.length - 5} more`,
                  );
                }
                vscode.window.showWarningMessage(
              ...
              console.log(
                `[DEBUG] syncBaseline: Starting individual uploads for ${actualTotal} files...`,
              );
              ...
              for (const relativePath of validFiles) {
                ...
                const devicePath = applyPathMappings(
                  relativePath,
                  rootPath,
                  pathMappings,
                );
                ...
                try {
                  ...
                  console.error(
                    `[DEBUG] syncBaseline: ✗ File no longer exists during individual upload: ${localPath}`,
                  );
                  failed++;
                  continue;
                }
                ...
                try {
                  console.log(
                    `[DEBUG] syncBaseline: Individual upload ${uploaded + 1}/${actualTotal}: ${localPath} -> ${devicePath}`,
                  );
                  ...
                  console.log(
                    `[DEBUG] syncBaseline: Executing cpToDevice: ${localPath} -> ${devicePath}`,
                  );

                  await withAutoSuspend(() =>
                    mp.cpToDevice(localPath, devicePath),
                  );
                  ...
                  uploaded++;
                  console.log(
                    `[DEBUG] syncBaseline: ✓ Individual upload ${uploaded}/${actualTotal} successful: ${relativePath}`,
                  );
                } catch (individualError: any) {
                  failed++;
                  console.error(
                    `[DEBUG] syncBaseline: ✗ Individual upload failed for ${relativePath}:`,
              ...
              console.log(
                `[DEBUG] syncBaseline: Individual uploads completed. ${uploaded} successful, ${failed} failed.`,
              );

              if (failed > 0) {
                console.warn(
                  `[DEBUG] syncBaseline: ${failed} files failed to upload individually`,
                );
              }
        ...
        console.log(
          `[DEBUG] syncBaseline: ✓ Manifest saved locally: ${manifestPath}`,
        );

        vscode.window.showInformationMessage(
          "Board: Sync all files (Local → Board) completed",
        );
        ...
        vscode.window.showErrorMessage(
          `Upload failed: ${error?.message ?? String(error)}`,
        );
      }
    }),

    vscode.commands.registerCommand(
      "mpyWorkbench.syncBaselineFromBoard",
      async () => {
        // Close the REPL terminal if open to avoid port conflicts
        if (isReplOpen()) {
          await disconnectReplTerminal();
        ...
        const rootPath = vscode.workspace
          .getConfiguration()
          .get<string>("mpyWorkbench.rootPath", "/");
        const deviceStats = await withAutoSuspend(() =>
          mp.listTreeStats(rootPath),
        );
        const matcher = await createIgnoreMatcher(ws.uri.fsPath);
        ...
        vscode.window.showInformationMessage(
          "Board: Sync all files (Board → Local) completed",
        );
        // Clear any diff/local-only markers after successful sync-all
        decorations.clear();
        tree.refreshTree();
    ...
    vscode.commands.registerCommand(
      "mpyWorkbench.autoSuspendLs",
      async (pathArg: string) => {
        listingInProgress = true;
        try {
          const usePyRaw = vscode.workspace
            .getConfiguration()
            .get<boolean>("mpyWorkbench.usePyRawList", false);
          return await withAutoSuspend(
            () => (usePyRaw ? listDirPyRaw(pathArg) : mp.lsTyped(pathArg)),
            { preempt: false },
          );
        } finally {
    ...
    vscode.commands.registerCommand(
      ...
      async () => {
        ...
        const rel = ws
          ? path.relative(ws.uri.fsPath, ed.document.uri.fsPath)
          : path.basename(ed.document.uri.fsPath);
        if (ws) {
          try {
            const matcher = await createIgnoreMatcher(ws.uri.fsPath);
            ...
            if (matcher(relPosix, false)) {
              vscode.window.showInformationMessage(
                `Upload skipped (ignored): ${relPosix}`,
              );
              return;
            }
          } catch {}
        ...
        try {
          await withAutoSuspend(() =>
            mp.uploadReplacing(ed.document.uri.fsPath, dest),
          );
          ...
          console.error(
            `[DEBUG] Failed to upload active file to board:`,
            uploadError,
          );
          vscode.window.showErrorMessage(
            `Failed to upload active file to board: ${uploadError?.message || uploadError}`,
          );
        }
      },
    ),
    vscode.commands.registerCommand(
      "mpyWorkbench.runActiveFile",
      runActiveFile,
    ),
    vscode.commands.registerCommand("mpyWorkbench.checkDiffs", () =>
      boardOperations.checkDiffs(),
    ),
    vscode.commands.registerCommand(
      "mpyWorkbench.syncDiffsLocalToBoard",
      async () => {
        const ws = vscode.workspace.workspaceFolders?.[0];
        if (!ws) {
          vscode.window.showErrorMessage("No workspace folder open");
        ...
        if (allDiffs.length === 0 && allLocalOnly.length === 0) {
          ...
          if (runCheck === "Check Differences Now") {
            ...
            if (newDiffs.length === 0 && newLocalOnly.length === 0) {
              vscode.window.showInformationMessage(
                "No differences found between local and board files.",
              );
              return;
            }
          } else {
        ...
        const filesSet = new Set(
          deviceStats.filter((e) => !e.isDir).map((e) => e.path),
        );
        const diffs = decorations
          .getDiffsFilesOnly()
        ...
        console.log(
          "- decorations.getDiffsFilesOnly():",
          decorations.getDiffsFilesOnly(),
        );
        console.log(
          "- decorations.getLocalOnlyFilesOnly():",
          decorations.getLocalOnlyFilesOnly(),
        );
    ...
    vscode.commands.registerCommand(
      "mpyWorkbench.syncDiffsBoardToLocal",
      async () => {
        ...
        const rootPath2 = vscode.workspace
          .getConfiguration()
          .get<string>("mpyWorkbench.rootPath", "/");
        // Get current diffs and filter to files by comparing with current device stats
        const deviceStats2 = await withAutoSuspend(() =>
          mp.listTreeStats(rootPath2),
        );
        const filesSet2 = new Set(
          deviceStats2.filter((e) => !e.isDir).map((e) => e.path),
        );
        const diffs2 = decorations
          .getDiffsFilesOnly()
          .filter((p) => filesSet2.has(p));
        ...
        decorations.clear();
        vscode.window.showInformationMessage(
          "Board: Diffed files downloaded from board and marks cleared",
        );
        tree.refreshTree();
      },
    ),
    vscode.commands.registerCommand(
      "mpyWorkbench.openFile",
      async (node: Esp32Node) => {
        if (node.kind !== "file") return;
        const ws = vscode.workspace.workspaceFolders?.[0];
        const rootPath = vscode.workspace
        ...
        if (ws) {
          ...
          if (isLocalOnly) {
            ...
            console.log(
              `[DEBUG] openFile (extension): Opening local-only file: ${abs}`,
            );
          } else {
            ...
            if (!fileExistsLocally) {
              console.log(
                `[DEBUG] openFile (extension): File not found locally, copying from board: ${node.path} -> ${abs}`,
              );
              try {
                await withAutoSuspend(() => mp.cpFromDevice(node.path, abs));
                console.log(
                  `[DEBUG] openFile (extension): Successfully copied file from board`,
                );
              } catch (copyError: any) {
                console.error(
                  `[DEBUG] openFile (extension): Failed to copy file from board:`,
                  copyError,
                );
                vscode.window.showErrorMessage(
                  `Failed to copy file from board: ${copyError?.message || copyError}`,
                );
                return; // Don't try to open the file if copy failed
              }
            } else {
              console.log(
                `[DEBUG] openFile (extension): File already exists locally: ${abs}`,
              );
            }
          }
          const doc = await vscode.workspace.openTextDocument(
          ...
          try {
            await withAutoSuspend(() =>
              mp.cpFromDevice(node.path, temp.fsPath),
            );
            ...
            console.error(
              `[DEBUG] openFile (extension fallback): Failed to copy file to temp location:`,
              copyError,
            );
            vscode.window.showErrorMessage(
              `Failed to copy file from board to temp location: ${copyError?.message || copyError}`,
            );
          }
        }
      },
    ...
    vscode.commands.registerCommand(
      "mpyWorkbench.delete",
      async (node: Esp32Node) => {
        const okBoard = await vscode.window.showWarningMessage(
          `Delete ${node.path} from board?`,
          { modal: true },
          "Delete",
        );
        ...
        await vscode.window.withProgress(
          ...
          async (progress, token) => {
            ...
            try {
              ...
              vscode.window.showInformationMessage(
                `Successfully deleted ${node.path} from board`,
              );
              tree.removeNode(node.path);
            } catch (err: any) {
              progress.report({ increment: 100, message: "Deletion failed!" });
              vscode.window.showErrorMessage(
                `Failed to delete ${node.path} from board: ${err?.message ?? String(err)}`,
              );
            }
          },
        );
    ...
    vscode.commands.registerCommand(
      "mpyWorkbench.deleteBoardAndLocal",
      async (node: Esp32Node) => {
        const okBoardLocal = await vscode.window.showWarningMessage(
          `Delete ${node.path} from board AND local workspace?`,
          { modal: true },
          "Delete",
        );
          ...
          async (progress, token) => {
            ...
            try {
              ...
              progress.report({
                increment: 50,
                message: isDir
                  ? "Removing directory from board..."
                  : "Removing file from board...",
              });
              await withAutoSuspend(() => mp.deleteAny(node.path));
              progress.report({
                increment: 70,
                message: "Board deletion complete!",
              });
              vscode.window.showInformationMessage(
                `Successfully deleted ${node.path} from board`,
              );
      ...
      await vscode.window.withProgress(
        ...
        async (progress, token) => {
          ...
          try {
            ...
            const items = await withAutoSuspend(() =>
              mp.listTreeStats(rootPath),
            );
            ...
            if (totalItems === 0) {
              ...
              vscode.window.showInformationMessage(
                `Board: No files found under ${rootPath}`,
              );
              return;
            }
            ...
            progress.report({
              increment: 20,
              message: `Found ${totalItems} items to delete...`,
            });

            // Usar nuestra nueva función para eliminar todo
            const result = await withAutoSuspend(() =>
              mp.deleteAllInPath(rootPath),
            );

            progress.report({
              increment: 80,
              message: "Verifying deletion...",
            });

            // Verificar lo que queda
            const remaining = await withAutoSuspend(() =>
              mp.listTreeStats(rootPath),
            );
            ...
            // Reportar resultados
            const deletedCount =
              (result as any).deleted_count ?? result.deleted.length;
            const errorCount =
              (result as any).error_count ?? result.errors.length;
            ...
            progress.report({ increment: 100, message: "Deletion failed!" });
            vscode.window.showErrorMessage(
              `Failed to delete files from board: ${error?.message ?? String(error)}`,
            );
          }
        },
      );
    ...
    vscode.commands.registerCommand(
      "mpyWorkbench.deleteAllBoardFromView",
      async () => {
    ...
    vscode.commands.registerCommand(
      "mpyWorkbench.syncBaselineFromView",
      async () => {
        setSkipIdleOnce();
        await vscode.commands.executeCommand("mpyWorkbench.syncBaseline");
      },
    ),
    vscode.commands.registerCommand(
      "mpyWorkbench.syncBaselineFromBoardFromView",
      async () => {
        setSkipIdleOnce();
        await vscode.commands.executeCommand(
          "mpyWorkbench.syncBaselineFromBoard",
    ...
    vscode.commands.registerCommand(
      "mpyWorkbench.checkDiffsFromView",
      async () => {
        setSkipIdleOnce();
        await vscode.commands.executeCommand("mpyWorkbench.checkDiffs");
      },
    ...
    vscode.commands.registerCommand(
      "mpyWorkbench.syncDiffsLocalToBoardFromView",
      async () => {
        setSkipIdleOnce();
        await vscode.commands.executeCommand(
          "mpyWorkbench.syncDiffsLocalToBoard",
    ...
    vscode.commands.registerCommand(
      "mpyWorkbench.syncDiffsBoardToLocalFromView",
      async () => {
        setSkipIdleOnce();
        await vscode.commands.executeCommand(
          "mpyWorkbench.syncDiffsBoardToLocal",
    ...
    vscode.commands.registerCommand(
      "mpyWorkbench.runActiveFileFromView",
      async () => {
        setSkipIdleOnce();
        await vscode.commands.executeCommand("mpyWorkbench.runActiveFile");
      },
    ),
    vscode.commands.registerCommand(
      "mpyWorkbench.openReplFromView",
      async () => {
        setSkipIdleOnce();
        ...
        const newName = await vscode.window.showInputBox({
          ...
          validateInput: (v) =>
            v && !v.endsWith("/") && !v.endsWith("\\")
              ? undefined
              : "Name must not end with / and cannot be empty",
        });
        if (!newName) return;
        const devicePath =
          baseDevice === "/"
            ? `/${newName.replace(/^\//, "")}`
            : `${baseDevice}/${newName.replace(/^\//, "")}`;
        try {
          ...
          try {
            ...
            vscode.window.showWarningMessage(
              `File created locally but upload to board failed: ${uploadError?.message || uploadError}`,
            );
          }
        } catch (err: any) {
          vscode.window.showErrorMessage(
            `Error creating file: ${err?.message ?? err}`,
          );
        }
      ...
      async (node: Esp32Node) => {
        ...
        const newName = await vscode.window.showInputBox({
          ...
          validateInput: (v) =>
            v && v !== oldName
              ? undefined
              : "Name must be different and not empty",
        });
        ...
        try {
          ...
          vscode.window.showErrorMessage(
            `Error renaming on board: ${err?.message ?? err}`,
          );
          return;
        }
        ...
        // Refresh tree
        const tree = vscode.extensions.getExtension("DanielBucam.mpy-workbench")
          ?.exports?.esp32Tree as { refreshTree: () => void };
  ...
  context.subscriptions.push(
    vscode.workspace.onDidSaveTextDocument(async (doc) => {
      ...
      if (!enabled) {
        ...
        if (now - lastLocalOnlyNotice > 5000) {
          vscode.window.setStatusBarMessage(
            "Board: Auto sync disabled — saved locally only (workspace)",
            3000,
          );
      ...
      const rootPath = vscode.workspace
        ...
        .get<string>("mpyWorkbench.rootPath", "/");
      const rel = path
        .relative(ws.uri.fsPath, doc.uri.fsPath)
        .replace(/\\/g, "/");
      try {
        const matcher = await createIgnoreMatcher(ws.uri.fsPath);
      ...
      // Apply path mappings for auto-upload
      const wsConfig = await readWorkspaceConfig(ws.uri.fsPath);
      const pathMappings = wsConfig.pathMappings;
      const deviceDest = applyPathMappings(rel, rootPath, pathMappings);
      try {
        ...
        vscode.window.showWarningMessage(
          `Board auto-upload failed for ${rel}: ${String((e as any)?.message ?? e)}`,
        );
      }
    }),
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand(
      "mpyWorkbench.toggleWorkspaceAutoSync",
      async () => {
        try {
          const ws = getWorkspaceFolder();
          ...
          vscode.window.showInformationMessage(
            `Workspace auto-sync on save is now ${cfg.autoSyncOnSave ? "ENABLED" : "DISABLED"}`,
          );
          try {
          ...
          vscode.window.showErrorMessage(
            "Failed to toggle workspace auto-sync: " + String(e),
          );
        }
      },
    ),
