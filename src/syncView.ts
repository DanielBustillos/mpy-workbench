...
export interface SyncActionNode {
  id: string;
  label: string;
  command: string;
}
...
export class SyncTree implements vscode.TreeDataProvider<SyncActionNode> {
  ...
  async getActionNodes(): Promise<SyncActionNode[]> {
    ...
    try {
      ...
      if (ws) {
        const cfg = require("fs").existsSync(
          ws.uri.fsPath + "/.mpy-workbench/config.json",
        )
          ? JSON.parse(
          ...
          : {};
        const enabled =
          typeof cfg.autoSyncOnSave === "boolean"
            ? cfg.autoSyncOnSave
            : vscode.workspace
                .getConfiguration()
                .get<boolean>("mpyWorkbench.autoSyncOnSave", false);
        autoSyncLabel = enabled
          ? "AutoSync: ON (click to disable)"
          : "AutoSync: OFF (click to enable)";
      }
    } catch {}
    return [
      {
      ...
      {
        id: "baseline",
        label: "Upload all files (Local → Board)",
        command: "mpyWorkbench.syncBaseline",
      },
      {
        id: "baselineFromBoard",
        label: "Download all files (Board → Local)",
        command: "mpyWorkbench.syncBaselineFromBoard",
      },
      {
        id: "checkDiffs",
        label: "Check for differences (local vs board)",
        command: "mpyWorkbench.checkDiffs",
      },
      {
        id: "syncDiffsLocalToBoard",
        label: "Sync changed Files Local → Board",
        command: "mpyWorkbench.syncDiffsLocalToBoard",
      },
      {
        id: "syncDiffsBoardToLocal",
        label: "Sync changed Files Board → Local",
        command: "mpyWorkbench.syncDiffsBoardToLocal",
      },
      {
        id: "deleteAllBoard",
        label: "Delete ALL files on Board",
